...
class BaseRole:
    ...
    def get_messages(ending_index: Optional[int] = None) -> list[Message]:
        ...
        if ending_index is not None:
            sandbox_database = sandbox_database.filter(pl.col("sandbox_message_index") <= ending_index)
        # Cast str back to enum
        return [Message(**row) for row in sandbox_database.drop("sandbox_message_index").to_dicts()]
    ...
    def messages_validation(cls, messages: list[Message]) -> None:
        ...
        if messages[-1].recipient != cls.role_type:
            raise KeyError(f"The last message should be addressed to {cls.role_type}, found {messages[-1].recipient}")
    ...
    def filter_messages(cls, messages: list[Message]) -> list[Message]:
        ...
        """
        return [message for message in messages if cls.role_type in cast("list[RoleType]", message.visible_to)]

    @classmethod
    def get_available_tools(cls) -> dict[str, Callable[..., Any]]:
        """Get the available tools for this role."""
        name_to_tool = get_current_context().get_available_tools(scrambling_allowed=cls.role_type == RoleType.AGENT)
        return {
            name: tool
